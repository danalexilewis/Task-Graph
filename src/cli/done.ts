...
async function getPlanWorktreeForTask(
  ...
  return { branch: row.worktree_branch, worktreePath: row.worktree_path };
}
...
export function doneCommand(program: Command) {
  program
    ...
    .option("--tokens-in <n>", "Input tokens consumed by this implementer session")
    .option("--tokens-out <n>", "Output tokens generated by this implementer session")
    .option("--tool-calls <n>", "Total tool call invocations (shell, read, write, etc.)")
    .option("--attempt <n>", "Which attempt this is (1 for first, 2 for first retry, etc.)")
        ...
        const singleResult = await q
          ...
          .andThen(() => {
            ...
            const parseOptionalInt = (
              ...
              name: string,
            ): { value: number | undefined; error?: ReturnType<typeof buildError> } => {
            ...
            const tokensInResult = parseOptionalInt(options.tokensIn, "tokens-in");
            if (tokensInResult.error) return err(tokensInResult.error);
            const tokensOutResult = parseOptionalInt(options.tokensOut, "tokens-out");
            if (tokensOutResult.error) return err(tokensOutResult.error);
            const toolCallsResult = parseOptionalInt(options.toolCalls, "tool-calls");
            ...
            if (tokensInResult.value !== undefined) selfReport.tokens_in = tokensInResult.value;
            if (tokensOutResult.value !== undefined) selfReport.tokens_out = tokensOutResult.value;
            if (toolCallsResult.value !== undefined) selfReport.tool_calls = toolCallsResult.value;
            if (attemptResult.value !== undefined) selfReport.attempt = attemptResult.value;
