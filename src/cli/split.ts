          ...
          return ResultAsync.fromPromise(
            (async () => {
              ...
              const originalDomainsResult = await q.select<{ doc: string }>(
                "task_doc",
                { columns: ["doc"], where: { task_id: resolved } },
              );
              ...
              for (const title of newTitles) {
                ...
                taskMappings.push({ original: resolved, new: newTaskId });
                ...
                const insertNewTaskEventResult = await q.insert("event", {
                  ...
                  task_id: newTask.task_id,
                  kind: "created",
                  body: jsonObj({ title: newTask.title, splitFrom: resolved }),
                  created_at: currentTimestamp,
                });
              ...
              const commitResult = await doltCommit(
                `task: split ${resolved} into ${newTitles.join(", ")}`,
                config.doltRepoPath,
