...
SELECT
  ...
  (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'project' AND COLUMN_NAME = 'is_benchmark') AS hc_project_is_benchmark,
  (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'task' AND COLUMN_NAME = 'is_benchmark') AS hc_task_is_benchmark
`;
...
interface BatchProbeRow {
  ...
  hc_project_is_benchmark: number;
  hc_task_is_benchmark: number;
}
  ...
  return doltSql(BATCH_PROBE_SQL, repoPath).andThen((rows) => {
    ...
    const result: MigrationProbeResult = {
      ...
      columns: {
        ...
        "project.is_benchmark": toBool(r.hc_project_is_benchmark),
        "task.is_benchmark": toBool(r.hc_task_is_benchmark),
      },
...
/** Add project and task is_benchmark columns if missing (idempotent). Used to mark benchmark plans and tasks. */
export function applyIsBenchmarkMigration(
  ...
  if (probe) {
    let migration = ResultAsync.fromSafePromise<void, AppError>(
      Promise.resolve(),
    );
    if (!probe.columns["task.is_benchmark"]) {
      migration = migration.andThen(() =>
        doltSql(
          "ALTER TABLE `task` ADD COLUMN is_benchmark TINYINT(1) NOT NULL DEFAULT 0",
          repoPath,
        )
          .map(() => {
            cache?.clear();
          ...
          .andThen(() =>
            doltCommit("db: add task.is_benchmark column", repoPath, noCommit),
          )
          .map(() => undefined),
      );
    }
    if (!probe.columns["project.is_benchmark"]) {
      migration = migration.andThen(() =>
        doltSql(
          "ALTER TABLE `project` ADD COLUMN is_benchmark TINYINT(1) NOT NULL DEFAULT 0",
          repoPath,
        )
          .map(() => {
            cache?.clear();
          ...
          .andThen(() =>
            doltCommit(
              "db: add project.is_benchmark column",
              repoPath,
              noCommit,
            ),
          )
  ...
  // Non-probe branch: add task first, then project
  return columnExists(repoPath, "task", "is_benchmark", cache)
    .andThen((exists) => {
      if (exists)
        return ResultAsync.fromSafePromise<void, AppError>(Promise.resolve());
      return doltSql(
        "ALTER TABLE `task` ADD COLUMN is_benchmark TINYINT(1) NOT NULL DEFAULT 0",
        repoPath,
      )
    ...
    .andThen(() => columnExists(repoPath, "project", "is_benchmark", cache))
    .andThen((exists) => {
      if (exists)
        return ResultAsync.fromSafePromise<void, AppError>(Promise.resolve());
      return doltSql(
        ...
        .andThen(() =>
          doltCommit("db: add project.is_benchmark column", repoPath, noCommit),
        )
        .map(() => undefined);
    });
}
...
export function ensureMigrations(
  ...
  return runMigrationProbe(repoPath).andThen((probe) => {
    ...
    return applyPlanRichFieldsMigration(repoPath, noCommit, cache, probe)
      ...
      .andThen(() =>
        applyIsBenchmarkMigration(repoPath, noCommit, cache, probe),
      )
      .map(() => {
