...
**Actual cause:** The import in `[src/export/mermaid.ts](src/export/mermaid.ts)` (and identically in `[src/export/dot.ts](src/export/dot.ts)`) is wrong:

```typescript
// mermaid.ts is at src/export/mermaid.ts
import { doltSql } from "../../db/connection";  // resolves to db/ — doesn't exist
...
- Fix: `[src/export/mermaid.ts](src/export/mermaid.ts)` — correct import path, remove dead imports, extract `formatMermaidGraph`
- Fix: `[src/export/dot.ts](src/export/dot.ts)` — same fixes, extract `formatDotGraph`
...
- Update: `[vitest.config.ts](vitest.config.ts)` — exclude integration from default run
- Update: `[package.json](package.json)` — add `test:integration` script
