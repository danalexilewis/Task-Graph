...
overview: Build a thin query builder layer in `src/db/query.ts` that wraps `doltSql` with `insert`, `select`, `update`, `insertEvent`, and `raw` helpers -- centralizing escaping, backtick-wrapping, NULL handling, datetime formatting, and JSON_OBJECT construction -- then migrate all 13 command files to use it.
todos:
  - id: query-builder
...
One new file: `[src/db/query.ts](src/db/query.ts)`. It exports a `query(repoPath)` factory that returns a builder object. The builder stays on the CLI transport (`doltSql`) -- no server, no new dependencies.
