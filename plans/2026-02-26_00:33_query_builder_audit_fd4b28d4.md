...
All command and domain code that touches the DB already goes through the thin query builder (`query(repoPath)` and `q.insert` / `q.update` / `q.select` / `q.count` / `q.raw`). The only direct `doltSql` user left is [src/db/migrate.ts](src/db/migrate.ts), which is correct—migrations run DDL (e.g. `CREATE TABLE`) that the builder does not support.
...
## 1. [src/cli/edge.ts](src/cli/edge.ts)
...
## 2. [src/domain/invariants.ts](src/domain/invariants.ts)
...
## 3. [src/export/graph-data.ts](src/export/graph-data.ts) (optional)
...
- **[src/cli/portfolio.ts](src/cli/portfolio.ts)** — Multi-table JOINs, GROUP BY, HAVING; only numeric `minFeatures` is interpolated. Correct as-is.
- **[src/cli/next.ts](src/cli/next.ts)** — Complex “next tasks” query; appropriate for `q.raw()`.
- **[src/cli/show.ts](src/cli/show.ts)** — Multiple raw queries for task detail, blockers, dependents, events; appropriate for `q.raw()`.
- **[src/db/migrate.ts](src/db/migrate.ts)** — DDL only; must keep using `doltSql` directly.
