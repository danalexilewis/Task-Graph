...
Both functions in [src/cli/utils.ts](src/cli/utils.ts) resolve the config path relative to `process.cwd()`:
...
This works fine for CLI usage (the user runs `tg` from their project root), but in tests `process.cwd()` is `` -- not the temporary test directory. So:

- `writeConfig()` tries to write to `.taskgraph/config.json` (wrong)
- `readConfig()` reads from `.taskgraph/config.json` (wrong)
...
Modify [src/cli/utils.ts](src/cli/utils.ts) so both functions accept an optional `basePath` argument that defaults to `process.cwd()`. This keeps CLI behavior unchanged while allowing tests to pass the temp directory:
...
- [src/export/graph-data.ts](src/export/graph-data.ts): `getGraphData(planId?, featureKey?, basePath?)`
- [src/export/mermaid.ts](src/export/mermaid.ts): `generateMermaidGraph(planId?, featureKey?, basePath?)`
- [src/export/dot.ts](src/export/dot.ts): `generateDotGraph(planId?, featureKey?, basePath?)`
...
- [graph-export.test.ts](__tests__/integration/graph-export.test.ts): Pass `context.tempDir` (or similar basePath) to `generateMermaidGraph`/`generateDotGraph` so they can read config from the temp directory
- [invariants-db.test.ts](__tests__/integration/invariants-db.test.ts): Guard `afterAll`
...
In [src/cli/init.ts](src/cli/init.ts) line 31, `dolt init` runs with `{ cwd: taskGraphPath }` which is `.taskgraph/`, but dolt creates its repo *in* the cwd. So the Dolt repo ends up at `.taskgraph/` itself rather than `.taskgraph/dolt/`. The e2e test checks for `.taskgraph/dolt/.dolt` but `tg init` creates `.taskgraph/.dolt`. Either the `init.ts` logic or the e2e test expectation needs to align. The fix depends on intended behavior: if `.taskgraph/dolt/` is the intended repo path, then `init.ts` should `mkdirSync(doltRepoPath)` and run `dolt init` with `{ cwd: doltRepoPath }` instead of `{ cwd: taskGraphPath }`.

---

## Files to Change

- **[src/cli/utils.ts](src/cli/utils.ts)** -- Add optional `basePath` param to `readConfig` and `writeConfig`
- **[src/export/graph-data.ts](src/export/graph-data.ts)** -- Add optional `basePath` param to `getGraphData`, pass to `readConfig`
- **[src/export/mermaid.ts](src/export/mermaid.ts)** -- Add optional `basePath` param to `generateMermaidGraph`
- **[src/export/dot.ts](src/export/dot.ts)** -- Add optional `basePath` param to `generateDotGraph`
- **[src/cli/init.ts](src/cli/init.ts)** -- Fix `dolt init` cwd to use `doltRepoPath` and create the directory first
- **[tests/integration/test-utils.ts](__tests__/integration/test-utils.ts)** -- Revert absolute dolt path, fix `writeConfig` call, fix `applyMigrations` await pattern
- **[tests/integration/graph-export.test.ts](__tests__/integration/graph-export.test.ts)** -- Pass basePath to generate functions, guard afterAll, fix doltSql await pattern
- **[tests/integration/invariants-db.test.ts](__tests__/integration/invariants-db.test.ts)** -- Guard afterAll, fix doltSql await pattern
- **[tests/e2e/core-flow.test.ts](__tests__/e2e/core-flow.test.ts)** -- No source changes needed (uses CLI via shell, so process.cwd() is the tempDir passed via `cwd`)
