...
todos:
  - id: env-precheck
    ...
    status: pending
  - id: run-tests
    content: Build taskgraph and run integration/e2e from repo root
    status: pending
...
- Run test commands from the repo root, where `[package.json](/Users/dan/repos/Task-Graph/package.json)` defines `test:e2e` and `test:integration`.

## Recommended command flow

1. From the repo root, export Dolt path explicitly:
...
- `[src/cli/init.ts](/Users/dan/repos/Task-Graph/src/cli/init.ts)` and `[src/db/connection.ts](/Users/dan/repos/Task-Graph/src/db/connection.ts)` now use `process.env.DOLT_PATH || "dolt"`; setting `DOLT_PATH` avoids PATH drift.
- Integration helper `[__tests__/integration/test-utils.ts](/Users/dan/repos/Task-Graph/__tests__/integration/test-utils.ts)` still shells `dolt init` and runs CLI commands that assume a valid package context.
...
1. In `[__tests__/e2e/core-flow.test.ts](/Users/dan/repos/Task-Graph/__tests__/e2e/core-flow.test.ts)`, invoke CLI directly via Node absolute path (`node dist/cli/index.js`) instead of `pnpm run start --filter ...` from temp `cwd`.
