...
The `[__tests__/db/query.test.ts](__tests__/db/query.test.ts)` file has several issues causing test failures and type errors:
...
### 1. `[__tests__/db/query.test.ts](__tests__/db/query.test.ts)`
...
Run `pnpm test` (or `npm test`) from repo root — all 75 tests should pass, including the 20 in query.test.ts.