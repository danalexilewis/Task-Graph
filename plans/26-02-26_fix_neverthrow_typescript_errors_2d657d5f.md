...
`readConfig()` in [utils.ts](src/cli/utils.ts) returns a synchronous `Result<Config, AppError>`. Every CLI command calls:
...
**Affected files**: [block.ts](src/cli/block.ts), [done.ts](src/cli/done.ts), [edge.ts](src/cli/edge.ts), [export.ts](src/cli/export.ts), [import.ts](src/cli/import.ts), [next.ts](src/cli/next.ts), [plan.ts](src/cli/plan.ts), [portfolio.ts](src/cli/portfolio.ts), [show.ts](src/cli/show.ts), [split.ts](src/cli/split.ts), [start.ts](src/cli/start.ts), [task.ts](src/cli/task.ts)
...
**Affected files**: Same as Category 1, plus [init.ts](src/cli/init.ts).
...
| File                                                       | Error                                                                                                                   | Fix                                                                                                                                                                                                                     |
| ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [next.ts](src/cli/next.ts)                 | Corrupted with inline line numbers from a bad write                                                                     | Rewrite the file cleanly                                                                                                                                                                                                |
| [split.ts](src/cli/split.ts)               | Uses `TaskStatusSchema` without importing it; uses `{ required: true }` on `.option()`                                  | Import `TaskStatusSchema`; change to `.requiredOption()`                                                                                                                                                                |
| [start.ts](src/cli/start.ts)               | Missing `err` import                                                                                                    | Add `err` to imports                                                                                                                                                                                                    |
| [task.ts](src/cli/task.ts)                 | Missing `err` import                                                                                                    | Add `err` to imports                                                                                                                                                                                                    |
| [init.ts](src/cli/init.ts)                 | `ErrorCode.UNKNOWN_ERROR` doesn't exist; `ResultAsync.fromPromise` receives `async () => {...}` (function, not Promise) | Add `UNKNOWN_ERROR` to `ErrorCode` enum; call the async IIFE to produce a Promise                                                                                                                                       |
| [plan.ts](src/cli/plan.ts)                 | Import path `../../db/escape` is wrong                                                                                  | Fix to `../db/escape`                                                                                                                                                                                                   |
| [migrate.ts](src/db/migrate.ts)            | `ResultAsync.fromPromise` receives `async () => {...}` (function, not Promise)                                          | Call the async IIFE: `(async () => { ... })()`                                                                                                                                                                          |
| [export.ts](src/cli/export.ts)             | `generateMermaidGraph` / `generateDotGraph` called with 3 args but only accept 2                                        | These functions use `readConfig()` internally via `getGraphData()`, so don't pass `repoPath`. Call with only `(planId, featureKey)`. Also, since they already return `ResultAsync`, no need for `asyncAndThen` wrapper. |
| [import.ts](src/cli/import.ts)             | Imports non-exported `ParsedPlan` from parser                                                                           | Export `ParsedPlan` from parser.ts                                                                                                                                                                                      |
| [importer.ts](src/plan-import/importer.ts) | `andThen` callback is async (same as Cat. 1)                                                                            | Refactor to use `ResultAsync.fromPromise` wrapper                                                                                                                                                                       |
