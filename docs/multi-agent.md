...
When multiple `/work` instances run, they coordinate via a **formation** defined in the **situation report (sitrep)**. To avoid every instance generating a sitrep at once, each instance first checks the **sitrep breadcrumb** (`.taskgraph/sitrep-breadcrumb.json`): if another agent is already generating (`state: making_sitrep`, `at` within 10 min), skip generation and go straight to task pull or read the existing sitrep. See [docs/leads/sitrep-breadcrumb.md](leads/sitrep-breadcrumb.md) for format and staleness rules (sitrep stale after 30 min). The sitrep (e.g. `reports/sitrep-YYYY-MM-DD-HHmm.md`) includes a **Formation** section that recommends which lead roles to fill and how many of each.
