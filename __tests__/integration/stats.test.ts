...
describe.serial("tg stats --plan shows plan summary and per-task elapsed table", () => {
  ...
  beforeAll(async () => {
    ...
    const ids = await getTaskIds(context, planId, ["Plan view task 1", "Plan view task 2"]);
  ...
  it("--json outputs planSummary with title and tasks array", async () => {
    ...
    const out = JSON.parse(stdout) as {
      planSummary: { title: string; task_count: number; total_elapsed_s: number | null };
      tasks: Array<{ hash_id: string; title: string; elapsed_s: number | null }>;
    };
    ...
    expect(out.tasks[0].elapsed_s).toBeGreaterThanOrEqual(out.tasks[1].elapsed_s ?? 0);
  });
  ...
  it("--plan also works with plan title (not just ID)", async () => {
    ...
    expect(exitCode).toBe(0);
    const out = JSON.parse(stdout) as { planSummary: { title: string } | null; tasks: unknown[] };
    expect(out.planSummary?.title).toBe("Stats Plan View Test");
  });
});
...
describe.serial("tg stats --timeline shows cross-plan execution history", () => {
  let context: Awaited<ReturnType<typeof setupIntegrationTest>> | undefined;
  ...
  beforeAll(async () => {
    ...
    const alphaIds = await getTaskIds(context, planAlphaId, ["Timeline alpha task 1"]);
    const betaIds = await getTaskIds(context, planBetaId, ["Timeline beta task 1"]);

    const q = query(context.doltRepoPath);
    const base = new Date();

    for (const taskId of [alphaIds["Timeline alpha task 1"], betaIds["Timeline beta task 1"]]) {
  ...
  it("human output shows a Plan Timeline header with plan titles", async () => {
    ...
    const { exitCode, stdout } = await runTgCli(`stats --timeline`, context.tempDir);
    expect(exitCode).toBe(0);
...
describe.serial("tg stats shows Token Usage section when self-report data exists", () => {
  let context: Awaited<ReturnType<typeof setupIntegrationTest>> | undefined;
  ...
  beforeAll(async () => {
    ...
    await q
      ...
      .then((r) => r._unsafeUnwrap());
    await q.update("task", { status: "doing" }, { task_id: taskId }).then((r) => r._unsafeUnwrap());
  ...
  it("--json output includes token_usage array with expected fields", async () => {
    ...
    const { exitCode, stdout } = await runTgCli(`stats --json`, context.tempDir);
    expect(exitCode).toBe(0);
    const out = JSON.parse(stdout) as {
...
describe.serial("tg stats does NOT show Token Usage section when no self-report data", () => {
  let context: Awaited<ReturnType<typeof setupIntegrationTest>> | undefined;
  ...
  it("--json output does NOT include token_usage key", async () => {
    ...
    const { exitCode, stdout } = await runTgCli(`stats --json`, context.tempDir);
    expect(exitCode).toBe(0);
    const out = JSON.parse(stdout) as { agent_metrics: unknown[]; token_usage?: unknown[] };
    // token_usage should be absent or empty when no self-report data
    const hasTokenUsage = "token_usage" in out && Array.isArray(out.token_usage) && out.token_usage.length > 0;
