...
describe("tg status focused views (--tasks, --projects, --initiatives) with --json", () => {
  ...
  it("tg status --tasks --json exits 0 and returns JSON array of task rows", async () => {
    ...
    const requiredKeys = ["task_id", "title", "plan_title", "status", "owner"];
  ...
  it("tg status --initiatives --json exits 0 and returns stub when initiative table missing", async () => {
    if (!context) throw new Error("Context not initialized");
    const { exitCode, stdout } = await runTgCli(
    ...
    expect(exitCode).toBe(0);
    const data = JSON.parse(stdout) as { stub?: boolean; message?: string };
    expect(data.stub).toBe(true);
    expect(typeof data.message === "string").toBe(true);
    expect(data.message).toContain("Initiative");
  }, 20000);
});
