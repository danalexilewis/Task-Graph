...
(0, vitest_1.describe)("Invariants (DB Dependent) Integration Tests", () => {
    ...
    (0, vitest_1.beforeAll)(async () => {
        ...
        'Intent', 
        NOW(), NOW()
      );`, context.doltRepoPath))._unsafeUnwrap();
    ...
    (0, vitest_1.it)("should return error if task is not found", async () => {
        ...
        (0, vitest_1.expect)(result._unsafeUnwrapErr().code).toBe(errors_1.ErrorCode.TASK_NOT_FOUND);
    });
    (0, vitest_1.it)("should return error if task is not in 'todo' status", async () => {
        ...
        (0, vitest_1.expect)(result._unsafeUnwrapErr().message).toContain("is not in 'todo' status");
    });
    (0, vitest_1.it)("should return error if task has unmet blockers", async () => {
        ...
        (0, vitest_1.expect)(result.isErr()).toBe(true);
        (0, vitest_1.expect)(result._unsafeUnwrapErr().code).toBe(errors_1.ErrorCode.TASK_NOT_RUNNABLE);
        (0, vitest_1.expect)(result._unsafeUnwrapErr().message).toContain("has 1 unmet blockers");
    });
    (0, vitest_1.it)("should return ok if task is runnable", async () => {
        if (!context)
            throw new Error("Context not initialized");
        // Mark taskId3 as done so taskId1 becomes runnable
        (await (0, connection_1.doltSql)(`UPDATE \`task\` SET status = 'done', updated_at = NOW() WHERE task_id = '${taskId3}';`, context.doltRepoPath))._unsafeUnwrap();
        const result = await (0, invariants_1.checkRunnable)(taskId1, context.doltRepoPath);
